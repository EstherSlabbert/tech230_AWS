...
### <a id="use-aws-cli">Use AWS CLI</a>
...
2. **Create a bucket** with: `aws s3api mb s3://tech230-esther-bucket --region eu-west-1`. (You may check this with `aws s3 ls`).
3. **Upload** a file to a bucket with: `aws s3 cp ~/sampletext.txt s3://tech230-esther-bucket`, verify upload with `aws s3 ls s3://tech230-esther-bucket`.
4. **Download** a file from a bucket with: `aws s3 cp s3://tech230-esther-bucket/sampletext.txt S3_downloads` verify on your system with `ls`.
...
6. **Delete a bucket** with: `aws s3 rb s3://tech230-esther-bucket`, to verify `aws s3 ls`.

### <a id="create-python-script-and-run-it">Create Python script and run it</a>
