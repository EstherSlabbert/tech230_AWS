...
Enter the required information. (See above section if you are stuck).
...
4. Create a Python script with `touch file-name.py`, then enter it with `nano create-bucket.py` and edit it to contain the appropriate python script for what you want to accomplish as found above in the [Python CRUD:](#python-crud) section. Save and exit the file.
5. Edit the file permissions so you can execute the file using: `chmod u+x file-name.py`.
6. Run the 'file-name.py' file using `python file-name.py` to run the commands you desire.
